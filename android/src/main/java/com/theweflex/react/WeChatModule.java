...
public class WeChatModule extends ReactContextBaseJavaModule implements IWXAPIEventHandler {
    ...
    public void launchMiniProgram(ReadableMap data, Callback callback) {
        ...
        if (data.hasKey("mini_id")) {
            // 填小程序原始id
            req.userName = data.getString("mini_id");
        }
        if (data.hasKey("mini_path")) {
            //拉起小程序页面的可带参路径，不填默认拉起小程序首页
            req.path = data.getString("mini_path");
        }
        // 可选打开 开发版，体验版和正式版
        ...
        callback.invoke(api.sendReq(req)?null:INVALID_ARGUMENT);
    }
